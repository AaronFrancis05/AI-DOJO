...
export default function TryoutAvatarPage() {
  ...
  return (
    <Suspense fallback={null}>
      <TryoutAvatarResolver />
    </Suspense>
  );
}

function TryoutAvatarResolver() {
  ...
  return <TryoutAvatarSession targetLanguage={params.targetLanguage} nativeLanguage={params.nativeLanguage} />;
}
    ...
    <div className="relative flex h-dvh flex-col overflow-hidden bg-dojo-canvas">
      <div className="relative z-20 flex items-center justify-between gap-2 px-4 sm:px-6 py-3 border-b border-dojo-border/60 shrink-0 backdrop-blur-md bg-dojo-surface/50">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-sm font-bold text-dojo-text-primary tracking-tight">Avatar Preview</span>
        </div>
        <button
          type="button"
      ...
      <div className="flex-1 relative z-10 overflow-hidden flex">
        ...
        <div className={`absolute top-0 left-0 bottom-0 z-30 w-80 max-w-full sm:w-96 flex flex-col bg-dojo-surface/95 backdrop-blur-xl border-r border-dojo-border/60 shadow-2xl transition-transform duration-300 ease-in-out ${chatOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex items-center justify-between px-4 py-3 border-b border-dojo-border/60 shrink-0">
            ...
            <button type="button" onClick={() => setChatOpen(false)} aria-label="Close conversation panel" className="flex h-8 w-8 items-center justify-center rounded-full hover:bg-dojo-border/20 text-dojo-text-muted hover:text-dojo-text-primary transition-colors">
              <X className="h-4 w-4" />
            </button>
          </div>
