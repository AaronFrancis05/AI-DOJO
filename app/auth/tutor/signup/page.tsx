/* ───────────────────────────────────────────────
   Tutor sign-up — the Tutor tab of /auth/signup.

   Not the same shape as the learner form and never
   will be: applying to teach collects a profile
   (languages, timezone, rate) as well as an
   account, and files it as pending. Verification
   is a human step, done from /admin.

   The tutor *sign-in* is the ordinary credential
   form at /auth/tutor/signin.
   ─────────────────────────────────────────────── */
      ...
      <div className="flex min-h-dvh items-center justify-center bg-dojo-canvas px-4">
        <div className="w-full max-w-md rounded-2xl border border-dojo-border bg-dojo-surface-raised p-8 text-center">
          ...
          <p className="mt-4 rounded-lg border border-dojo-border bg-dojo-surface px-4 py-3 text-sm leading-relaxed text-dojo-text-muted">
            ...
            <Link href="/auth/tutor/signin" className="font-medium text-dojo-accent hover:underline">
              /auth/tutor/signin
            </Link>{' '}
    ...
    <div className="min-h-dvh bg-dojo-canvas">
      <div className="mx-auto w-full max-w-2xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          ...
          {/* The tutor's own door. One account system underneath — `users.role`
              decides what it opens — but a returning tutor now has a page that
              says so, instead of being told on the learner form that this was
              not their door. */}
          <span className="text-sm text-dojo-text-muted">
            Already have an account?{' '}
            <Link href="/auth/tutor/signin" className="font-semibold text-dojo-accent hover:underline">
              Sign in
            </Link>
          </span>
        </div>

        {/* Same switch as the learner form: this *is* the Tutor tab of
            /auth/signup, it just collects a teaching profile as well. */}
        <div className="mx-auto max-w-sm">
          <AuthRoleTabs role="tutor" mode="signup" />
        </div>
