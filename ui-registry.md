...
| `lib/roleplay/sentence-split.ts` | `findSentenceEnd(buffer,isFinal)` / `insideSpan(buffer,i)` — where a streaming reply may be cut. A boundary inside an unclosed ⟦ ⟧ span is skipped: an `⟦Bonjour !` fragment has no closing partner, so `splitIntoLangSpans` reads it as native and the target-language line is spoken in the learner's own voice. Kept out of `tts.ts` so it is testable without the Speech SDK (`sentence-split.test.ts`). |
