...
## 2026-08-28 (the real reason admin was a learner: four unapplied migrations)

The auth fixes earlier today were correct but were not the cause. The cause was in the database, and the Vercel runtime logs named it outright: `[sync-user] failed … column "auth_user_id" does not exist` on **every authenticated request**.

- **Production shipped code that expected a column the database did not have.** `drizzle/0048_friendly_cammi.sql` adds `users.auth_user_id`; it had never been applied. `syncUser` selects that column first thing, so it threw on every call.
- **That is what made an admin a learner.** `getAuthUser()` catches a `syncUser` failure and falls back to the *auth provider's* id. `getUserRole()` / `requireRole()` then look up `users.id = <auth id>` — a row that does not exist — so the role read returned nothing and every gate treated the account as a learner. `getAuthUserReadOnly()` resolves by **email** instead, which is why the sidebar showed the right person while the role was wrong: the two halves of the app were identifying the user by different keys, and only one of them was broken.
- **`syncUser` also *creates* the row.** With it throwing, the account had no `users` row at all — which is why `POST /api/auth/admin/claim` answered **404**, not 403. That 404 is also proof `ADMIN_EMAILS` *is* correctly set in Vercel: the route checks the allowlist (403) before it reads the row (404), so reaching 404 means the address passed the allowlist.
- **Four migrations were pending: 0045, 0046, 0047, 0048.** Applied with `npm run db:migrate`, which handled them correctly — the watermark bug was already fixed, and its dual-digest (raw + LF-normalized) matching is what correctly recognised 0033/0035/0036 as applied. An ad-hoc raw-only hash check called those three pending; they were not. **Trust the script's own pending list over a re-implementation of it.**
- The moment the column existed, the next request from the signed-in browser synced the row into existence on its own. No data was lost and nothing needed deleting.
- Snapshot of `drizzle.__drizzle_migrations` taken before the run: `backups/drizzle-migrations-2026-08-28T06-27-43-434Z.json` (44 rows). Three of those rows are duplicate records of already-applied migrations carrying hand-rounded future `created_at` values — harmless to the current script, since selection is a hash set difference, but they are why a watermark-based selector broke here in the first place.
- **Ordering lesson:** the deploy went out ahead of the migration. Code that reads a new column reached production before the column did, and the failure surfaced as an authorisation bug — a role that silently degraded — rather than as an obvious 500.

## 2026-08-29 (instant live rooms, curriculum-anchored titles, notified bookings)

Six connected gaps in live tutoring, fixed together. Migration `drizzle/0049_unknown_flatman.sql` adds `went_live_at` to `class_sessions` and `assessment_sessions`; applied.

- **The unit footer on `/courses/[slug]` had never once fired.** It keys on `classByUnit`, but `CreateRoomForm` never sent `courseId`/`unitId` — both POST routes accepted them and the console simply omitted them, so every class and assessment ever created had `unit_id = NULL`. The feature looked implemented and was dead on arrival. The console now carries a Course → Level → Unit picker, and `resolveRoomAnchor()` verifies the unit really belongs to the course (previously any integer that parsed was stored).
- **`status: 'live'` existed but meant nothing.** `canJoinBooking()` was purely time-window based, so a tutor opening a room early left every learner reading "This session has not opened yet". `'live'` now always allows and `'completed'` always refuses, ahead of the clock — this one change is what makes instant rooms work at all. Live rooms also survive the one-hour cutoff in both list routes, or a 90-minute class would vanish from the page mid-lesson.
- **`went_live_at` is an idempotency guard, not a timestamp for its own sake.** A tutor toggling live → scheduled → live must not announce themselves to their cohort twice. Both the instant-create path and the first `PATCH` to `'live'` stamp it and fan out; nothing does so if it is already set.
- **Going live reuses `resolveAudience()` and must keep doing so.** `lib/tutors/live.ts` resolves the pinned course's cohort, or all the tutor's learners when unpinned. A second membership query here is the exact failure the audience module was written to prevent — the console preview and the delivery disagreeing.
- **An instant class has no roster by definition,** so the class token route enrols on the way in rather than 403-ing. `enrolLearner()` was extracted from the enrol route so both paths take the same seat under the same advisory lock; the class PATCH also unions the enrolled roster into the announcement, since someone who signed up for one class may not be one of this tutor's learners by any other route.
- **AI-examined assessments close themselves.** Nobody is in the room to end one. Two clauses are load-bearing and neither is obvious: `done > 0`, or a room that goes live with an empty queue closes before anyone arrives; and `status = 'live'` re-checked in the `UPDATE ... WHERE` with `.returning()`, or two simultaneous last finishers both close it and both notify the tutor.
- **The booking loop was silent end to end** — no notification on request, confirm, complete or cancel. That, not the state machine, was why bookings felt opaque: `requested → confirmed → completed` is a fine model that nobody was ever told about. Also added inline Confirm/Decline to the console's Schedule tab, whose only previous route to confirming was opening the room page.
- Lint caught two real defects in the new console code (`react-hooks/set-state-in-effect`): the title prefill and the course reset were derived state in effects. Both moved into the change handlers, which is also the better semantics — clearing the unit now keeps the title the tutor was shown.

## 2026-08-29 (review pass on the live-rooms work: five races and a repeat notification)

Follow-up to the commit above. Every finding was a check-then-act I had written the same way in four places.

- **`wentLiveAt` was read, then written.** Both PATCH routes decided `isFirstOpen` from the row they had already loaded and then updated unconditionally, so two racing PATCHes both saw null and both announced to the cohort. The claim is now `UPDATE ... WHERE id = ? AND went_live_at IS NULL RETURNING id` — only the request that gets a row back announces. The status write stays unconditional, or a later re-open would stop applying.
- **The AI auto-close checked the queue outside any lock.** Between "nobody is pending" and the UPDATE, a learner could call `startInterview` — which takes `pg_advisory_xact_lock(assessmentId)` — and end up with a live interview attached to a room that had just closed. Both halves now run under that same lock: `closeAssessmentIfDrained()` wraps the drain check and the close in one locked transaction, and `startInterview` re-reads the room's status under the lock.
- **That status re-check is "closed", not "not live", and the distinction matters.** A scheduled AI assessment inside its join window has never been PATCHed live, and `canJoinBooking` allows it; refusing anything that is not `'live'` would have broken the ordinary case. Only `completed` and `cancelled` refuse.
- **A repeated PATCH sent a second "your lesson is confirmed".** The write is idempotent, the notification is not — a second one reads as a second lesson. Guarded on `found.booking.status !== status`.
