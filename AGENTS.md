...
- **Localization pattern:** scenario/situation/vocab content has a base row (English scenario text + target-language vocab) plus optional rows in `scenarioLocalizations` / `situationLocalizations` / `vocabularyLocalizations` keyed by `languageCode`, resolved through `lib/localization.ts` (`getScenarioLocalization` for native-language UI text, `getTargetScenarioLocalization`/`getTargetSituationLocalization`/`getTargetVocabLocalizations` for target-language content, `applyScenarioLocalization`/`applySituationLocalization` for merging). Any new localized content must go through this existing mechanism, not a parallel one.
